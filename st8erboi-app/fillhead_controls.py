...
def create_fillhead_ancillary_controls(parent, send_fillhead_cmd):
    ...
    # Refactored to use a more compact 2-column grid layout
    ...
    # Refactored buttons from pack to a 2x3 grid to save horizontal space
    ...
    ttk.Button(jog_btn_frame, text="X-",
               command=lambda: send_fillhead_cmd(f"MOVE_X -{jog_cmd_str(fh_jog_dist_mm_var.get())}")).grid(row=0,
                                                                                                           column=0,
                                                                                                           sticky='ew',
                                                                                                           padx=(0, 1),
                                                                                                           pady=(0, 1))
    ttk.Button(jog_btn_frame, text="X+",
               command=lambda: send_fillhead_cmd(f"MOVE_X {jog_cmd_str(fh_jog_dist_mm_var.get())}")).grid(row=1,
                                                                                                          column=0,
                                                                                                          sticky='ew',
                                                                                                          padx=(0, 1))

    ttk.Button(jog_btn_frame, text="Y-",
               command=lambda: send_fillhead_cmd(f"MOVE_Y -{jog_cmd_str(fh_jog_dist_mm_var.get())}")).grid(row=0,
                                                                                                           column=1,
                                                                                                           sticky='ew',
                                                                                                           padx=1,
                                                                                                           pady=(0, 1))
    ttk.Button(jog_btn_frame, text="Y+",
               command=lambda: send_fillhead_cmd(f"MOVE_Y {jog_cmd_str(fh_jog_dist_mm_var.get())}")).grid(row=1,
                                                                                                          column=1,
                                                                                                          sticky='ew',
                                                                                                          padx=1)

    ttk.Button(jog_btn_frame, text="Z-",
               command=lambda: send_fillhead_cmd(f"MOVE_Z -{jog_cmd_str(fh_jog_dist_mm_var.get())}")).grid(row=0,
                                                                                                           column=2,
                                                                                                           sticky='ew',
                                                                                                           padx=(1, 0),
                                                                                                           pady=(0, 1))
    ttk.Button(jog_btn_frame, text="Z+",
               command=lambda: send_fillhead_cmd(f"MOVE_Z {jog_cmd_str(fh_jog_dist_mm_var.get())}")).grid(row=1,
                                                                                                          column=2,
                                                                                                          sticky='ew',
                                                                                                          padx=(1, 0))
    ...
    home_btn_frame.grid_columnconfigure((0, 1, 2), weight=1)  # Make buttons expand equally

    home_cmd_str = lambda axis: f"HOME_{axis} {fh_home_torque_var.get()} {fh_home_distance_mm_var.get()}"
    ttk.Button(home_btn_frame, text="Home X", command=lambda: send_fillhead_cmd(home_cmd_str("X"))).grid(row=0,
                                                                                                         column=0,
                                                                                                         sticky='ew',
                                                                                                         padx=(0, 2))
    ttk.Button(home_btn_frame, text="Home Y", command=lambda: send_fillhead_cmd(home_cmd_str("Y"))).grid(row=0,
                                                                                                         column=1,
                                                                                                         sticky='ew',
                                                                                                         padx=2)
    ttk.Button(home_btn_frame, text="Home Z", command=lambda: send_fillhead_cmd(home_cmd_str("Z"))).grid(row=0,
                                                                                                         column=2,
                                                                                                         sticky='ew',
                                                                                                         padx=(2, 0))
