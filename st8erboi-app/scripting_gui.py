...
class ScriptRunner:
    ...
    def _handle_wait(self, args, line_num):
        if not args:
            self.status_callback(f"Error on L{line_num}: WAIT requires a duration in milliseconds.", line_num)
            self.is_running = False
            return False
        try:
            duration_ms = float(args[0])
            self.status_callback(f"L{line_num}: Waiting for {duration_ms} ms...", line_num)
            ...
            self.status_callback(f"Error on L{line_num}: Invalid duration for WAIT.", line_num)
            self.is_running = False
            return False
            ...
            for sub_cmd_str in sub_commands:
                ...
                if device == "script":
                    if command_word == "WAIT":
                        if not self._handle_wait(args, line_num): break
