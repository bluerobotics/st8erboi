...
def create_pinch_jog_section(parent, send_cmd_func, variables):
    """
    Creates and populates the jog controls for the pinch motor (M2).
    This version is corrected to use mm-based units and reflects that
    "close" is a positive move and "open" is a negative move.
    """
    ...
    # Labels and variables use mm-based units
    tk.Label(jog_params_frame, text="Dist (mm):", bg=jog_params_frame['bg'], fg="white", font=font_small).grid(row=0,
                                                                                                               column=0,
                                                                                                               sticky="e",
                                                                                                               pady=1,
    ...
    tk.Label(jog_params_frame, text="Accel (mm/s²):", bg=jog_params_frame['bg'], fg="white", font=font_small).grid(
        row=1, column=0, sticky="e", pady=1, padx=(0, 5))
    ttk.Entry(jog_params_frame, textvariable=variables['jog_pinch_accel_mms2_var'], width=field_width,
              font=font_small).grid(
    ...
    pinch_jog_cmd_str = lambda dist: (
        f"PINCH_JOG_MOVE {dist} "
        f"{variables['jog_pinch_velocity_mms_var'].get()} "
        f"{variables['jog_pinch_accel_mms2_var'].get()} "
    ...
    ttk.Button(jog_buttons_area, text="Pinch Close", style="Jog.TButton",
               command=lambda: send_cmd_func(pinch_jog_cmd_str(variables['jog_pinch_dist_mm_var'].get()))).grid(row=0,
                                                                                                                column=1,
                                                                                                                sticky="ew",
                                                                                                                padx=(
                                                                                                                    1,
...
def create_manual_controls_panel(parent, command_funcs, shared_gui_refs):
    ...
    if 'jog_pinch_velocity_mms_var' not in shared_gui_refs: shared_gui_refs[
        'jog_pinch_velocity_mms_var'] = tk.StringVar(value="5.0")
    if 'jog_pinch_accel_mms2_var' not in shared_gui_refs: shared_gui_refs['jog_pinch_accel_mms2_var'] = tk.StringVar(
        value="25.0")
    if 'pinch_jog_torque_percent_var' not in shared_gui_refs: shared_gui_refs[
        'pinch_jog_torque_percent_var'] = tk.StringVar(value="20")
...
def create_manual_controls_display(parent, command_funcs, shared_gui_refs):
    ...
    # --- Trigger Button (Canvas for vertical text) ---
