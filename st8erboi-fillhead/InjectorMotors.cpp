...
float globalTorqueLimit    = 2.0f;   // Default overall limit, overridden by moveInjectorMotors's param
float torqueOffset         = -2.4f;  // Global offset, settable by USER_CMD_SET_TORQUE_OFFSET
...
// These are not directly used by moveInjectorMotors if commands always supply a torque limit.
...
void abortInjectorMove(){
	ConnectorM0.MoveStopAbrupt();
	ConnectorM1.MoveStopAbrupt();
}

void moveInjectorMotors(int stepsM0, int stepsM1, int torque_limit_percent, int velocity_sps, int accel_sps2) {
	if (!motorsAreEnabled) {
	...
	// Set the globalTorqueLimit that checkInjectorTorqueLimit() will use for this operation
...
void movePinchMotor(int stepsM3, int stepsM1, int torque_limit_percent, int velocity_sps, int accel_sps2) {
	if (!motorsAreEnabled) {
		sendToPC("MOVE BLOCKED: Motors are disabled.");
		return;
	}
